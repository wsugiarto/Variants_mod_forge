...
public class CharcoalStaffBolt extends AbstractStaffBolt{
    private int life;

    public CharcoalStaffBolt(EntityType<? extends CharcoalStaffBolt> p, Level level){
        super(p, level);
    }

    // This should be constructor for non abstract
    public CharcoalStaffBolt(Level level, LivingEntity owner, double x, double y, double z){
        this(ModEntity.CHARCOAL_BOLT.get(), level);
        this.setOwner(owner);
