...
public class StoneLauncherProjectile extends AbstractStaffBolt{
    float damage = 4.0F;
    private int pierces;
    ParticleOptions particle = ParticleTypes.WAX_ON;

    public StoneLauncherProjectile(EntityType<? extends StoneLauncherProjectile> p, Level level){
        super(p, level);
    }

    // This should be constructor for non abstract
    public StoneLauncherProjectile(Level level, LivingEntity owner, double x, double y, double z){
        this(ModEntity.STONE_LAUNCHER_PROJECTILE.get(), level);
        this.setOwner(owner);
    ...
    public void tick(){
        ...
        if (this.level().getBlockStates(this.getBoundingBox()).noneMatch(BlockBehaviour.BlockStateBase::isAir)) { // If in a block
            ...
            this.setDeltaMovement(this.getDeltaMovement().add(0, -0.3, 0)); // gravity
            this.tickDespawn();
        } else { // in air
            this.setDeltaMovement(vec3.scale((double)1.0F)); // this changes friction
            this.setDeltaMovement(this.getDeltaMovement().add(0, -0.1, 0)); // gravity
            this.tickDespawn();
