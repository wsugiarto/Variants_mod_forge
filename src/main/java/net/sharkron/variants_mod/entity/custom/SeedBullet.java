...
public class SeedBullet extends AbstractStaffBolt{
    private int life;

    public SeedBullet(EntityType<? extends SeedBullet> p, Level level){
        super(p, level);
    }

    // This should be constructor for non abstract
    public SeedBullet(Level level, LivingEntity owner, double x, double y, double z){
        this(ModEntity.SEED_BULLET.get(), level);
        this.setOwner(owner);
