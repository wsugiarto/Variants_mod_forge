...
public class TorchTombProjectile extends AbstractHurtingProjectile{
    private int life;

    public TorchTombProjectile(EntityType<? extends TorchTombProjectile> p, Level level){
        super(p, level);
    }

    // This should be constructor for non abstract
    public TorchTombProjectile(Level level, LivingEntity owner, double x, double y, double z){
        this(ModEntity.TORCH_TOMB_PROJECTILE.get(), level);
        this.setOwner(owner);
    ...
    protected void onHitEntity(EntityHitResult hit) {
        ...
        if (entity instanceof LivingEntity livingentity) {
            hit.getEntity().hurt(this.damageSources().mobProjectile(this, livingentity), 0.0F);
