...
public class TorchTombProjectileRenderer extends EntityRenderer<TorchTombProjectile>{
   ...
   private final EvokerFangsModel<TorchTombProjectile> model;

   public TorchTombProjectileRenderer(EntityRendererProvider.Context p_174100_) {
      super(p_174100_);
      this.model = new EvokerFangsModel<>(p_174100_.bakeLayer(ModelLayers.EVOKER_FANGS));
   }

   public void render(TorchTombProjectile p_114528_, float p_114529_, float p_114530_, PoseStack p_114531_, MultiBufferSource p_114532_, int p_114533_) {
      float f = 1.0F;
      if (f != 0.0F) {
         float f1 = 2.0F;
         if (f > 0.9F) {
   ...
   public ResourceLocation getTextureLocation(TorchTombProjectile p_114526_) {
