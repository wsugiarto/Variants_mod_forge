...
public class Blowpipe extends Item{
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (level.isClientSide || !hasAmmo(player)) {
            ...
            SeedBullet proj1 = new SeedBullet(level, player, look.x, look.y, look.z);

            // level.playSound((Player)null, player.getX(), player.getY(), player.getZ(), SoundEvents.CROSSBOW_SHOOT, SoundSource.PLAYERS, 1.0F, 1.0F);
    ...
    private boolean hasAmmo(Player player){
        ...
        for(int i = 0; i < player.getInventory().getContainerSize(); i++){
            ...
            if(!stack.isEmpty() && stack.is(ModTags.Items.SEEDS)){
                return true;
            }
        }
        return false;
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(Component.literal("Consumes Seed"));
